...
const Validation = () => {
    ...
    const { wallet, address, setStep, signOutGoogle } = useContext(Context);
    ...
    useEffect(() => {
        setWalletConnected(address !== undefined && address !== "" && address !== null)
    }, [address])
        ...
        <div className="text-main font-semibold flex flex-col mb-8 gap-4 justify-center">
            {
                ...
                <React.Fragment>
                    ...
                    <div className="flex flex-col gap-2">
                        <span>Google account connection {googleConnected ? "✅" : "❌"}</span>
                        <span className=" text-l1-color">{session?.user?.identifier}</span>
                        {
                            googleConnected && 
                            <div className="mt-4"><Button type="secondary" onClick={() => signOutGoogle()}>Logout Google</Button>
</div>
                        }
                    </div>
                </React.Fragment>
